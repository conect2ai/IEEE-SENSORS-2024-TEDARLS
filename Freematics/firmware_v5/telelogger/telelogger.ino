...
byte byte_y_predp = 0x1B6;

float totalMemoryUsage = 0;
float minMemoryUsage = FLT_MAX;  // Começa com o valor máximo possível
float maxMemoryUsage = FLT_MIN;  // Começa com o valor mínimo possível
int sampleCount = 0;

void updateMemoryMetrics() {
    float currentMemoryUsage = heap_caps_get_free_size(MALLOC_CAP_8BIT);  // Ou use MALLOC_CAP_SPIRAM para PSRAM

    // Atualiza o total acumulado e o número de amostras
    totalMemoryUsage += currentMemoryUsage;
    sampleCount++;

    // Verifica se o valor atual é o novo mínimo ou máximo
...
void printMemoryMetrics() {
    ...
    Serial.print("Average Memory Usage: ");
    Serial.println(averageMemoryUsage);

    Serial.print("Minimum Memory Usage: ");
    Serial.println(minMemoryUsage);

    Serial.print("Maximum Memory Usage: ");
    Serial.println(maxMemoryUsage);
}
